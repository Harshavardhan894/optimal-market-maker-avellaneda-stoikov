...
def run_simulation(config: SimulationConfig) -> Dict:
    ...
    effective_gamma = config.gamma * (1.35 if config.enable_adversarial else 1.0)
    effective_delta = config.delta + (0.08 if config.enable_adversarial else 0.0)
    strategy = AvellanedaStoikovMarketMaker(
        config=MarketMakerConfig(
            gamma=effective_gamma,
            sigma=config.sigma,
            T=config.T,
            base_delta=effective_delta,
    ...
    for t in range(1, config.ticks + 1):
        ...
        # 3) Send orders to LOB (with latency feature)
